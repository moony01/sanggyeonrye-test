...
(function (global) {
  ...
  var RESULT_TYPE_COLORS = {
    freepass: { border: "rgba(212, 165, 165, 1)" }, // 로즈골드
    moonjeon: { border: "rgba(139, 69, 87, 1)" }, // 딥로즈
  };

  var RESULT_BAR_COLORS = {
    freepass: "rgba(212, 165, 165, 1)", // 로즈골드
    moonjeon: "rgba(139, 69, 87, 1)", // 딥로즈
  };

  var RESULT_BAR_BG_COLORS = {
    freepass: "rgba(212, 165, 165, 0.2)",
    moonjeon: "rgba(139, 69, 87, 0.2)",
  };
  ...
  // 다국어 구글 검색 문구 (6개 언어 지원) - 상견례 테스트용
  var SEARCH_TEXTS = {
    ko: "상견례 얼굴상 테스트",
    en: "meeting in-laws face test",
    ja: "相見礼 顔診断テスト",
    zh: "相亲对象面相测试",
    vi: "test tướng mặt gặp gia đình",
    id: "tes wajah pertemuan keluarga",
    default: "sanggyeonrye face test",
  };

  // 다국어 CTA 텍스트 (6개 언어 지원) - 상견례 테스트용
  var CTA_TEXTS = {
    ko: "나도 테스트 하기!",
    en: "Try the test!",
    ja: "テストしてみる！",
  ...
  function drawPercentageBars(ctx, predictions, startY) {
    ...
    for (var i = 0; i < predictions.length && i < 4; i++) {
      ...
      ctx.textBaseline = "middle";
      ctx.fillText(
        pred.agency.toUpperCase(),
        paddingLeft + labelWidth - 15,
        y + barHeight / 2,
      );
      ...
      var bgColor =
        AGENCY_BAR_BG_COLORS[agencyKey] || "rgba(200, 200, 200, 0.2)";
  ...
  function generateResultImage(data) {
    ...
    var imagePromise = userImageSrc
      ? loadImage(userImageSrc).catch(function () {
          return null;
        })
